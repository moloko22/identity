...
export class AuthService {
    ...
    generateToken = async (tokenData: JwtTokenData, type: JwtTokenType = JwtTokenType.ACCESS): Promise<string> => {
        const refreshTokenOptions = {
            secret: JWT_CONFIG.secret,
            expiresIn: '7d',
        }

        const token = type === JwtTokenType.ACCESS ?
    ...
    async signIn(tokenData: User): Promise<AuthResultDto> {
        ...
        const refreshToken = await this.generateToken(tokenPayload, JwtTokenType.REFRESH)
